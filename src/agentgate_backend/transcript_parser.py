...
Output: plain text + content_type marker (text/thinking/tool_use/tool_result).
No Telegram-specific formatting (MarkdownV2, expandable quotes) in this module —
channel formatting is handled by the gateway layer.
...
class TranscriptParser:
    ...
    _RE_SYSTEM_TAGS = re.compile(
        r"<(bash-input|bash-stdout|bash-stderr|local-command-caveat|system-reminder|task-notification|tool-use-feedback)"
    )
    ...
    def _format_tool_result_text(cls, text: str, tool_name: str | None = None) -> str:
        """Format tool result text with statistics summary (plain text).

        Shows relevant statistics for each tool type.
        No channel-specific formatting — the gateway layer handles that.
        """
        if not text:
            return ""
        ...
        if tool_name == "Read":
            ...
            if line_count > 0:
                return f"  ⎿  Output {line_count} lines\n{text}"
            return text
            ...
            if line_count > 0:
                return f"  ⎿  Agent output {line_count} lines\n{text}"
            return text
            ...
            results = text.count("\n\n") + 1 if text else 0
            return f"  ⎿  {results} search results\n{text}"
                    ...
                    if btype == "tool_result":
                        ...
                        if is_interrupted:
                            ...
                            if result_text:
                                ...
                                entry_text += f"\n  ⎿  Error: {error_summary}"
                                # If multi-line error, append full text
                                if "\n" in result_text:
                                    entry_text += "\n" + result_text
                            else:
                                entry_text += "\n  ⎿  Error"
