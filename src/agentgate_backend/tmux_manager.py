...
class TmuxManager:
    ...
    async def _accept_trust_dialog(
        self,
        window_id: str,
        *,
        timeout: float = 10.0,
        ...
        """Poll pane content and press Enter when the trust dialog appears.

        Claude Code shows "Is this a project you trust?" on first entry to a
        new directory.  The default highlighted option is "Yes", so pressing
        Enter accepts it.

        Args:
            window_id: tmux window ID (e.g. "@21").
            timeout: Maximum seconds to wait for the dialog.
            poll_interval: Seconds between capture-pane polls.

        Returns:
            True if the dialog was detected and accepted (or never appeared
            within the timeout — which means it was already trusted).
        """
        ...
        while _time.monotonic() < deadline:
            ...
            if pane_text and "trust" in pane_text.lower():
                await self.send_keys(window_id, "Enter", enter=False, literal=False)
                logger.info("Accepted trust dialog in window %s", window_id)
                return True
            await asyncio.sleep(poll_interval)

        logger.warning(
            "Trust dialog not detected in window %s within %.0fs (may already be trusted)",
            window_id,
            timeout,
        )
        return False
    ...
    def get_session(self) -> libtmux.Session | None:
        ...
        try:
            ...
            logger.error(
                "TmuxManager: failed to get session '%s': %s",
                self.session_name,
                e,
                exc_info=True,
            )
    ...
    async def capture_pane(self, window_id: str, with_ansi: bool = False) -> str | None:
        ...
        if with_ansi:
            ...
            try:
                ...
                logger.error(
                    f"Unexpected error capturing pane {window_id}: {e}", exc_info=True
                )
                return None
        ...
        def _sync_capture() -> str | None:
            ...
            try:
                ...
                logger.error(f"Failed to capture pane {window_id}: {e}", exc_info=True)
                return None
        ...
        if literal and enter:
            ...
            def _send_literal(chars: str) -> bool:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to send keys to window {window_id}: {e}", exc_info=True
                    )
                    return False
            ...
            def _send_enter() -> bool:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to send Enter to window {window_id}: {e}",
                        exc_info=True,
                    )
                    return False
    ...
    async def kill_window(self, window_id: str) -> bool:
        ...
        def _sync_kill() -> bool:
            ...
            try:
                ...
                logger.error(f"Failed to kill window {window_id}: {e}", exc_info=True)
                return False
    ...
    async def create_window(
        ...
        claude_command_override: str | None = None,
    ) -> tuple[bool, str, str, str]:
        """Create a new tmux window and optionally start an agent.

        Args:
            work_dir: Working directory for the new window
            window_name: Optional window name (defaults to directory name)
            start_claude: Whether to start the agent command
            claude_command_override: Override the default agent command
                (e.g. "opencode -m dashscope/qwen-plus" instead of "claude")
        ...
        # Create window in thread (sync part: tmux new_window + send claude cmd)
        def _create_and_start() -> tuple[bool, str, str, str]:
            ...
            try:
                ...
                if start_claude:
                    pane = window.active_pane
                    if pane:
                        cmd = claude_command_override or config.claude_command
        ...
        # Accept trust dialog asynchronously (after the sync thread returns)
        ok, msg, wname, wid = result
        if ok and start_claude and wid:
            await self._accept_trust_dialog(wid)
