...
Runs an async polling loop that:
  ...
  4. Parses entries via TranscriptParser and calls on_new_output callback.

Output is exposed via HTTP API (/api/output), not pushed directly to any channel.
The callback signature is: on_new_output(window_name: str, messages: list[dict])
...
# Output callback: window_name (str) + list of message dicts
OutputCallback = Callable[[str, list[dict]], Awaitable[None]]
...
class SessionMonitor:
    ...
    Output callback signature:
        async def on_new_output(window_name: str, messages: list[dict]) -> None

    Each message dict has keys: role, text, content_type, timestamp,
    tool_use_id (optional), tool_name (optional).
    """
    ...
    def __init__(
        ...
        # Simplified output callback: on_new_output(window_name, messages)
        ...
        # Reverse map: session_id -> window_key (for output routing)
        self._session_to_window: dict[str, str] = {}

    def set_output_callback(self, callback: OutputCallback) -> None:
        """Set the output callback for new messages.

        Callback signature: async def on_new_output(window_name: str, messages: list[dict])
        """
    ...
    async def _load_current_session_map(self) -> dict[str, str]:
        ...
        (e.g. "agentgate:@12"). Old-format keys ("agentgate:window_name") are
        also accepted so that sessions running before a code upgrade continue
    ...
    async def _detect_and_cleanup_changes(self) -> dict[str, str]:
        ...
        # Update last known map and reverse index
        self._last_session_map = current_map
        self._session_to_window = {sid: wid for wid, sid in current_map.items()}
    ...
    async def _monitor_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if new_messages and self._output_callback:
                    # Group messages by window for the output callback
                    # window_key -> list of message dicts
                    window_messages: dict[str, list[dict]] = {}
                    for msg in new_messages:
                        status = "complete" if msg.is_complete else "streaming"
                        preview = msg.text[:80] + ("..." if len(msg.text) > 80 else "")
                        logger.info("[%s] session=%s: %s", status, msg.session_id, preview)

                        window_key = self._session_to_window.get(msg.session_id, "")
                        if not window_key:
                            continue
                        if window_key not in window_messages:
                            window_messages[window_key] = []
                        window_messages[window_key].append({
                            "role": msg.role,
                            "text": msg.text,
                    ...
                    # Emit per-window batches to the output callback
                    for window_key, msgs in window_messages.items():
                        try:
                            await self._output_callback(window_key, msgs)
                        except Exception as e:
                            logger.error(
                                "Output callback error for window %s: %s",
                                window_key, e, exc_info=True,
                            )
                elif new_messages:
